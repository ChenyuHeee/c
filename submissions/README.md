...
2. Choose one of the following formats:
   - **Traditional**: Create your handle folder under `submissions/week-1/` (e.g. `submissions/week-1/alice/solution.c`)
   - **Simplified**: Create your handle folder directly under `submissions/` with a file named `solution.c` (e.g. `submissions/alice/solution.c`)
3. Add a C source file named `solution.c`. Must read from stdin and write to stdout.
...
1. Fork 本仓库。
2. 选择以下格式之一：
   - **传统格式**：在 `submissions/week-1/` 下新建以自己昵称命名的目录（例如 `submissions/week-1/alice/solution.c`）
   - **简化格式**：直接在 `submissions/` 下新建以自己昵称命名的目录，并创建名为 `solution.c` 的文件（例如 `submissions/alice/solution.c`）
3. 添加一个名为 `solution.c` 的 C 源文件，程序需从标准输入读取并向标准输出打印结果。
4. 发起 PR 到 `main`。
...
- For simplified format, the filename **must** be `solution.c`.
- For traditional format, any `.c` filename is allowed, but `solution.c` is preferred.
